...
const MODMAIL_THREAD_PREFIX = "📩・";
const MODMAIL_CLOSE_BUTTON_ID = "zaqerai:modmail-close:";
...
const activeModmails = new Map<string, ActiveModmail>();
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("customize")
        .setDescription("Customize the ModMail message and thread appearance.")
        .addStringOption((option) =>
          option
            .setName("title")
            .setDescription("Title of the new ModMail embed.")
            .setRequired(false),
        )
        .addStringOption((option) =>
          option
            .setName("description")
            .setDescription("Description of the new ModMail embed.")
            .setRequired(false),
        )
        .addStringOption((option) =>
          option
            .setName("footer")
            .setDescription("Footer shown on the ModMail embed.")
            .setRequired(false),
        )
        .addStringOption((option) =>
          option
        ...
        .addStringOption((option) =>
          option
            ...
            .setDescription("Prefix used for new ModMail threads.")
            .setRequired(false),
        )
        .addStringOption((option) =>
          option
            .setName("open_message")
            .setDescription("DM sent to the user when ModMail opens.")
            .setRequired(false),
        ),
    ),
];
...
const DEFAULT_MODMAIL_CONFIG: Omit<ModmailConfig, "forumId" | "ownerId"> = {
  title: "📩 New ModMail",
  description:
    "A new ModMail conversation has been opened.\\n\\n" +
    "**User:** {username}\\n" +
    "**User ID:** \`{userid}\`\\n\\n" +
    "Messages from this user will appear below.",
  footer: "Zaqerai Optimizations ModMail",
  embedColor: DEFAULT_EMBED_COLOR,
  threadPrefix: "📩・",
  openMessage:
    "📩 **ModMail opened!**\\n\\n" +
    "Your message has been sent to the support team. " +
    "You will receive staff replies here.",
};
...
async function getModmailConfig(
  ...
  try {
    ...
    const config: ModmailConfig = {
      forumId: rawConfig.forumId,
      ownerId: rawConfig.ownerId,
      title: rawConfig.title ?? DEFAULT_MODMAIL_CONFIG.title,
      description:
        rawConfig.description ?? DEFAULT_MODMAIL_CONFIG.description,
      footer: rawConfig.footer ?? DEFAULT_MODMAIL_CONFIG.footer,
      embedColor:
        rawConfig.embedColor ?? DEFAULT_MODMAIL_CONFIG.embedColor,
      threadPrefix:
        rawConfig.threadPrefix ?? DEFAULT_MODMAIL_CONFIG.threadPrefix,
      openMessage:
        rawConfig.openMessage ?? DEFAULT_MODMAIL_CONFIG.openMessage,
    };
...
function modmailCloseButton(
  userId: string,
): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
      .setCustomId(`${MODMAIL_CLOSE_BUTTON_ID}${userId}`)
      .setLabel("🔒 Close ModMail")
      .setStyle(ButtonStyle.Danger),
  );
}

const modmailCreationLocks = new Map<string, Promise<ThreadChannel | null>>();
...
async function createModmailThread(
  ...
  const existingLock = modmailCreationLocks.get(userId);

  if (existingLock) {
    return existingLock;
  }

  const creation = (async (): Promise<ThreadChannel | null> => {
    ...
    const existing = activeModmails.get(userId);

    if (existing) {
      const existingChannel = guild.channels.cache.get(existing.threadId);
      ...
      modmailUsers.delete(existing.threadId);
    }

    const thread = await forum.threads.create({
      name: `${config.threadPrefix}${safeChannelName(username, "user")}`,
      message: {
        embeds: [
          new EmbedBuilder()
        ...
        components: [modmailCloseButton(userId)],
      },
    });

    activeModmails.set(userId, {
      guildId: guild.id,
      userId,
      threadId: thread.id,
    });

    modmailUsers.set(thread.id, userId);
    ...
    await thread.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(firstMessage.content || "*No text content*")
          .setFooter({
            text: `User • ${username}`,
          })
          .setTimestamp(),
  ...
  modmailCreationLocks.set(userId, creation);

  try {
    return await creation;
  } finally {
    modmailCreationLocks.delete(userId);
  }
}

async function handleModmailDM(message: Message): Promise<void> {
  ...
  for (const guild of message.client.guilds.cache.values()) {
    ...
    const active = activeModmails.get(userId);

    if (active && active.guildId === guild.id) {
      const existingChannel = guild.channels.cache.get(active.threadId);

      if (existingChannel?.isThread()) {
        thread = existingChannel as ThreadChannel;
      } else {
        activeModmails.delete(userId);
        modmailUsers.delete(active.threadId);
      }
    }

    if (!thread) {
      thread = await createModmailThread(
        guild,
        userId,
...
async function handleModmailStaffMessage(message: Message): Promise<void> {
  if (message.author.bot || !message.guild || !message.channel.isThread()) {
    return;
  }

  const thread = message.channel as ThreadChannel;

  let userId = modmailUsers.get(thread.id);

  // Recover the user ID after a bot restart by reading the ModMail starter embed.
  if (!userId) {
    try {
      const starterMessage = await thread.fetchStarterMessage().catch(() => null);
      const footer = starterMessage?.embeds[0]?.footer?.text ?? "";
      const match = footer.match(/User ID:\\s*(\\d{17,20})/);
      ...
      logger.error(
        ...
        "Failed to recover ModMail user",
      );
    }
  }
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `Support • ${message.author.tag}`,
      })
      .setTimestamp();
    ...
    logger.error(
      ...
      "Failed to send ModMail reply",
    );

    await message.reply(
      "❌ I couldn't send this reply to the user. Their DMs may be disabled.",
    ).catch(() => {});
  }
}

async function handleModmailClose(interaction: Interaction): Promise<void> {
  if (!interaction.isButton()) {
    return;
  }

  if (!interaction.customId.startsWith(MODMAIL_CLOSE_BUTTON_ID)) {
    return;
  }

  const userId = interaction.customId.slice(
    MODMAIL_CLOSE_BUTTON_ID.length,
  );
  ...
  if (!threadId) {
    await interaction.reply({
      content: "❌ This ModMail thread could not be found.",
      ephemeral: true,
    });
    return;
  }
  ...
  if (!active || active.threadId !== threadId) {
    await interaction.reply({
      content: "❌ This ModMail conversation is no longer active.",
      ephemeral: true,
    });
    return;
  }
  ...
  try {
    ...
    await user.send(
      "🔒 **ModMail closed**\\n\\n" +
        ...
        "You can send me another message anytime to open a new ModMail conversation.",
    );
  } catch {}
  ...
  await interaction.reply({
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          "This ModMail conversation has been closed.\\n\\n" +
            "The user has been notified. This thread will be deleted shortly.",
        )
        .setTimestamp(),
    ],
  });
  ...
  setTimeout(() => {
    void interaction.channel?.delete("ModMail conversation closed");
  }, 5000);
}
  ...
  if (interaction.commandName === "modmail") {
    ...
    if (subcommand === "setup") {
      ...
      const existingConfig =
        (await getModmailConfig(interaction.guild.id)) ?? {
          forumId: forum.id,
          ownerId: MODMAIL_OWNER_ID,
      ...
      await interaction.reply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              `ModMail is now active.\\n\\n` +
                `**Forum:** ${forum}\\n` +
                `**System:** DM → Forum → Staff → DM\\n\\n` +
                `Users can now DM the bot to contact the support team.`,
            )
            .setFooter({
              text: "Zaqerai Optimizations ModMail",
            })
            .setTimestamp(),
        ],
        ephemeral: true,
    ...
    if (subcommand === "customize") {
      ...
      const config = await getModmailConfig(interaction.guild.id);

      if (!config) {
        await interaction.reply({
          content:
            "❌ ModMail is not configured yet. Run \`/modmail setup\` first.",
          ephemeral: true,
        });
        return;
      }
      ...
      const footer = interaction.options.getString("footer");
      const embedColor = interaction.options.getString("embed_color");
      const threadPrefix = interaction.options.getString("thread_prefix");
      const openMessage = interaction.options.getString("open_message");
      ...
      if (embedColor) {
        config.embedColor = parseEmbedColor(embedColor);
      }

      if (threadPrefix) {
        config.threadPrefix = threadPrefix;
      }

      if (openMessage) {
        config.openMessage = openMessage.replace(/\\n/g, "\\n");
      }
      ...
      await interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor(config.embedColor)
            .setTitle("⚙️ ModMail Customized")
            .setDescription(
              "Your ModMail settings have been saved.\\n\\n" +
...
export function startDiscordBot(): void {
  ...
  client.once("clientReady", async (readyClient) => {
    ...
    // ZAQERAI_ROTATING_STATUS
    const rotatingStatuses = [
      "discord.gg/zaqerai",
      "Zaqerai working",
      "the Zaqerai community",
      "for new members",
      "Zaqerai services",
    ];
    ...
    const updateBotStatus = () => {
      readyClient.user.setPresence({
        activities: [
          {
            name: rotatingStatuses[statusIndex],
            type: 3,
          },
        ],
        status: "online",
    ...
    updateBotStatus();
    setInterval(updateBotStatus, 30_000);
  ...
  client.on("messageCreate", async (message) => {
    try {
      ...
      if (message.channel.isThread()) {
        await handleModmailStaffMessage(message);
      }
    } catch (error) {
      logger.error(
        { err: error },
        "Discord ModMail message handling failed",
      );
    }
  });
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      if (interaction.isChatInputCommand()) {
        ...
        interaction.isButton() &&
        interaction.customId.startsWith(MODMAIL_CLOSE_BUTTON_ID)
      ) {
        await handleModmailClose(interaction);
      }
    } catch (error) {
